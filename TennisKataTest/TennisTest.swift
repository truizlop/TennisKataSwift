...
class TennisTest: XCTestCase {
    ...
    func testScoring() {
        ...
        property("Given advantage, when advantaged player scores, the game is over in its favor") <- forAll { (player: Player) in
            let actual = scoreWhenAdvantage(advantagedPlayer: player, winner: player)
            let expected = Score.game(player)
            return actual == expected
        }
        
        property("Given advantage, when the other player scores, the result is deuce") <- forAll { (player: Player) in
            let actual = scoreWhenAdvantage(advantagedPlayer: player, winner: player.other)
            let expected = Score.deuce
            return actual == expected
        }
        
        property("Given deuce, when a player scores, it gets advantaged") <- forAll { (player: Player) in
            let actual = scoreWhenDeuce(winner: player)
            let expected = Score.advantage(player)
            return actual == expected
        }
        
        property("Given a player with 40 points, when it scores, the game is over") <- forAll { (current: FortyData) in
            let actual = scoreWhenForty(winner: current.player, current: current)
            let expected = Score.game(current.player)
            return actual == expected
        }
        
        property("Given a result 40 - 30, when the latter player scores, the result is deuce") <- forAll { (player: Player) in
        ...
        property("Given a result 40 - < 30, when the latter player scores, its points are incremented") <- forAll { (player: Player) in
            return forAll(lessThan30Gen.map{FortyData(player: player, otherPlayerPoint: $0)}){ (current: FortyData) in
        ...
        property("Given a player has 30 points, when it scores, it gets 40") <- forAll { (pointData: PointData, player: Player) in
        ...
        property("Given both players have <30 points, when one scores, its points are incremented") <- forAll(Player.arbitrary, pointDataGen) { (player: Player, current: PointData) in
            let actual = scoreWhenPoints(winner: player, current: current)
    ...
    func testTennisRules() {
        property("A game with less than 4 balls isn't over") <- forAll(self.generateSequenceOf(lengthSmallerThan: 4)) { (sequence: [Player]) in
        ...
        property("A game with less than 7 balls can't have any player with advantage") <- forAll(self.generateSequenceOf(lengthSmallerThan: 7)) { (sequence: [Player]) in
            let actual = scoreSequence(wins: sequence)
            return !self.isAdvantage(actual)
        }
        
        property("A game with more than 4 balls can't be points") <- forAll(self.generateSequenceOf(lengthGreaterThan: 4)) { (sequence: [Player]) in
            let actual = scoreSequence(wins: sequence)
            return !self.isPoints(actual)
        }
        
        property("A game with more than 5 balls can't be forty") <- forAll(self.generateSequenceOf(lengthGreaterThan: 5)) { (sequence: [Player]) in
            let actual = scoreSequence(wins: sequence)
